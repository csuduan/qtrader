...
**TradingEngine** (`src/trading_engine.py`)
- 所有交易操作的中央协调器
...
- 使用 `src.api.responses` 中的 `success_response()` 和 `error_response()`
...
| `src/strategy/strategy_manager.py` | 策略生命周期和事件路由 |
| `src/strategy/base_strategy.py` | 策略基类 |
| `src/api/routes/` | FastAPI路由处理器 |
| `src/scheduler.py` | APScheduler任务管理 |
| `src/api/schemas.py` | API响应模型 |
