...
class TqGateway(BaseGateway):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # 启动tq主线程
            self._tq_thead = threading.Thread(
                target=self._tq_run, name=f"TqSdk_Thread", daemon=True
            )
    ...
    def _tq_run(self):
        ...
        try:
            ...
            if broker_type == "kq":
                account = TqKq()
            elif broker_type == "tq" or broker_type == "real":
                account = TqAccount(
    ...
    def subscribe_bars(self, symbol: str, interval: str) -> bool:
        ...
        if std_symbol is None:
            logger.error(f"无法识别合约代码: {symbol}")
            return False
        ...
        if self.api is None:
            return False
        contract = self.contracts.get(std_symbol)
        if not contract:
    ...
    def _query_and_save_contracts(self, update_date: str) -> None:
        ...
        try:
            ...
            for index, item in symbol_infos.iterrows():
                symbol = (
                    item.instrument_id.split(".")[1]
                    if "." in item.instrument_id
            ...
            logger.info(
                f"从API加载了 {len(self.contracts)} 个合约信息，准备保存 {len(contracts_to_save)} 个到数据库"
            )
    ...
    def refresh_contracts(self) -> bool:
        ...
        try:
            if not self.connected or not self.api:
                logger.error("TqSdk未连接，无法刷新合约信息")
                return False

            update_date = datetime.now().strftime("%Y-%m-%d")
            logger.info(f"开始强制刷新合约信息 (日期: {update_date})")

            # 重置合约缓存日期，强制从API查询
            self._contracts_update_date = None
            # 调用查询方法
            self._query_and_save_contracts(update_date)

            logger.info(f"强制刷新合约信息完成，共 {len(self.contracts)} 个合约")
            return True
        except Exception as e:
            logger.exception(f"强制刷新合约信息失败: {e}")
            return False
    ...
    def get_positions(self) -> Dict[str, PositionData]:
        ...
        for item in self._positions.values():
            position = self._convert_position(item)
    ...
    def get_quotes(self) -> Dict[str, TickData]:
        """获取行情数据(兼容,返回原始格式)"""
        return {
            symbol: self._convert_tick(quote)
    ...
    def _convert_account(self, account: Account) -> AccountData:
        ...
        return AccountData(
            ...
            pre_balance=account.static_balance or 0,
            static_balance=account.static_balance or 0,
    ...
    def _convert_position(self, pos: Position) -> PositionData:
        """转换持仓数据"""
        contract = self.contracts.get(pos.instrument_id)
        position =  PositionData(
            symbol=pos.instrument_id,
            exchange=self._parse_exchange(pos.exchange_id),
            multiple=contract.multiple if contract else 0,
