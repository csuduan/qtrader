...
class BaseGateway(ABC):
    ...
    def __init__(self):
        """初始化Gateway"""
        self.md_connected: bool = False  # 行情连接状态
        self.td_connected: bool = False  # 交易连接状态
        self.trading_day: Optional[str] = None
        self.contracts: Dict[str, ContractData] = {}
        self._contracts_update_date: Optional[str] = None
        self.positions: Dict[str, PositionData] = {}
        # 开仓限制配置 {symbol: min_open_volume}
        self._open_limit: Optional[Dict[str, int]] = None
        logger.info(f"{self.gateway_name} Gateway 初始化完成")


    def _fill_open_limit(self, contract: ContractData) -> None:
        """
        填充开仓限制到合约信息
        ...
        Args:
            contract: 合约数据对象
        """
        if not self._open_limit:
            return

        # 匹配品种（合约代码去掉数字）
        # 提取品种（如 rb2505 -> rb）
        match = re.match(r'^([a-zA-Z]+)', contract.symbol)
        if match:
            product = match.group(1).lower()
            min_volume = self._open_limit.get(product)
            if min_volume is not None:
                contract.min_open_volume = min_volume
                logger.debug(f"合约 {contract.symbol} 设置最小开仓手数: {min_volume}")

    @property
    def connected(self) -> bool:
        """网关是否已连接（行情和交易都连接时为True）"""
        return self.md_connected and self.td_connected
