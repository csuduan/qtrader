...
class SimpleSplitStrategy(BaseSplitStrategy):
    ...
    def split(self, pos: PositionData | None) -> int:
        ...
        min_volume = cmd.min_volume  # 最小开仓手数
        ...
        if cmd.offset != Offset.OPEN:
            ...
            if pos and cmd.offset == Offset.CLOSE:
                ...
                if pos_volume < cmd.volume:
                    raise ValueError(f"持仓不足，实际持仓：{pos_volume}手")
            ...
            #开仓
            max_volume = max(cmd.volume_per_order, min_volume) #确保最大单次手数不小于最小开仓手数
            # 检查总手数是否满足最小开仓手数要求（仅开仓时检查）
            if total_volume < min_volume:
                raise ValueError(
                    f"开仓手数{total_volume}小于最小开仓手数{min_volume}，无法下单"
                )
            ...
            while remaining_volume > 0:
                ...
                if remaining_volume <= max_volume:
                    # 剩余手数小于等于最大单次手数，直接全部报单
                    volume = remaining_volume
                else:
                    # 剩余手数大于最大单次手数，需要拆单
                    after_this = remaining_volume - max_volume
                    if after_this < min_volume :
                        # 如果拆单后剩余的手数小于最小开仓手数，则本次拆单手数为剩余手数
                        volume = remaining_volume
                    else:
                        volume = max_volume
                self._order_queue.append(SplitOrder(volume=volume, offset=cmd.offset, delay_seconds=0))
        ...
        logger.info(
            ...
            f"最小开仓手数{min_volume},拆单手数列表{[order.volume for order in self._order_queue]}"
        )
...
class OrderCmd:
    ...
    def __init__(
        ...
        self.slip = slip  # 滑点
        self.min_volume = 1  # 最小开仓手数，默认1，拆单时会根据合约配置设置
    ...
    def _cancel(self, reason: str) -> None:
        ...
        if not "全部完成" in reason:
            logger.error(f"报单指令取消：{self.cmd_id} {reason}")
