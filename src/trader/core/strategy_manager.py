...
class StrategyManager:
    ...
    def init(
        self, config_path: str, trading_engine, event_engine: Optional[EventEngine] = None
    ) -> bool:
        """
        初始化策略管理器
        ...
        Args:
            ...
            event_engine: EventEngine实例（可选，如果不提供则使用 trading_engine.event_engine）
        ...
        try:
            ...
            self.event_engine = event_engine or trading_engine.event_engine
    ...
    def _register_events(self):
        ...
        self.event_engine.register(
            EventTypes.TICK_UPDATE, lambda e: self._dispatch_event("on_tick", e.data)
        )
        self.event_engine.register(
            EventTypes.KLINE_UPDATE, lambda e: self._dispatch_event("on_bar", e.data)
        )
        self.event_engine.register(
            EventTypes.ORDER_UPDATE, lambda e: self._dispatch_event("on_order", e.data)
        )
        self.event_engine.register(
            EventTypes.TRADE_UPDATE, lambda e: self._dispatch_event("on_trade", e.data)
        )
    ...
    def load_config(
        self, config_path: str, trading_engine=None, event_engine: Optional[EventEngine] = None
    ) -> bool:
        """加载策略配置（兼容旧接口）"""
        import yaml
    ...
    def load_strategies_from_config(
        self, trading_engine=None, event_engine: Optional[EventEngine] = None
    ):
        """从配置文件加载并实例化策略（兼容旧接口）"""
        from src.trader.strategy import get_strategy_class
    ...
    def buy(
        ...
        try:
            ...
            if order_id:
                ...
                logger.info(
                    f"策略 [{strategy_id}] 买入: {symbol} {volume}手 @{price or '市价'} -> 订单ID: {order_id}"
                )
            return order_id
        except Exception as e:
    ...
    def sell(
        ...
        try:
            ...
            if order_id:
                ...
                self.order_strategy_map[order_id] = strategy_id
                logger.info(
                    f"策略 [{strategy_id}] 卖出: {symbol} {volume}手 @{price or '市价'} -> 订单ID: {order_id}"
                )
            return order_id
        except Exception as e:
