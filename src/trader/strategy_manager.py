...
class StrategyManager:
    ...
    def _load_strategies(self) -> None:
        ...
        for name, config in self.strategies_configs.items():
            ...
            try:
                ...
                strategy.trading_engine = self.trading_engine
        ...
        for strategy in self.strategies.values():
            ...
            for symbol, position in positions.items():
                if position.pos_long > 0:
                    position.hold_price_long = position.last_price or 0
                    position.pos_long_yd = position.pos_long
                    position.pos_long_td = 0
                if position.pos_short > 0:
                    position.hold_price_short = position.last_price or 0
    ...
    def save_strategy_position(self, position: "PositionData") -> bool:
        """
        保存策略持仓到数据库
        ...
        try:
            ...
            if account_id:
                position.extras["account_id"] = account_id
    ...
    def get_strategy_position(self, strategy_id: str, symbol: str) -> Optional["PositionData"]:
        """
        获取策略指定合约的持仓
