...
class BaseStrategy:
    ...
    def init_positions(self, positions: List[PositionData]) -> None:
        """
        初始化策略持仓（从数据库加载）
        ...
        for pos in positions:
            ...
            logger.info(
                ...
                f"多仓={pos.pos_long}({pos.pos_long_td or 0}/{pos.pos_long_yd or 0}), "
                f"空仓={pos.pos_short}({pos.pos_short_td or 0}/{pos.pos_short_yd or 0})"
            )
    ...
    def _get_or_create_position(self, symbol: str) -> PositionData:
        """
        获取或创建指定合约的持仓对象
        ...
        if symbol not in self._positions:
            ...
            contract = self.trading_engine.get_contract(symbol)
            if not contract:
                logger.error(f"策略 [{self.strategy_id}] 未找到合约信息: {symbol}")
                raise Exception(f"策略 [{self.strategy_id}] 未找到合约信息: {symbol}")
    ...
    def get_position(self, symbol: Optional[str] = None) -> Optional[PositionData]:
        """
        获取指定合约的持仓信息
    ...
    def get_all_positions(self) -> Dict[str, PositionData]:
        ...
        Returns:
            Dict[str, PositionData]: {symbol -> position}
        """
    ...
    async def _handle_normal_close(self, signal: Signal) -> None:
        ...
        if pos > 0:
            ...
            await self.send_order_cmds([exit_cmd])
    ...
    async def _handle_lock_close(self, signal: Signal) -> None:
        ...
        if pos_net > 0:
            ...
            await self.send_order_cmds([lock_cmd])
        elif pos_net < 0:
            # 锁仓：净头寸为空，需要开多
            lock_cmd = OrderCmd(
                symbol=self.param.symbol,
    ...
    async def _handle_normal_open(self, signal: Signal) -> None:
        ...
        if pos < self.volume:
            ...
            await self.send_order_cmds([entry_cmd])
    ...
    async def _handle_lock_open(self, signal: Signal) -> None:
        ...
        if signal.side == 1:
            ...
            elif position.pos < self.volume:
                #净多不足目标手数了，优先平空(昨)，剩余开多
                target_volume = self.volume - position.pos
                close_volume  = min(target_volume, position.pos_short_yd)
                open_volume = target_volume - close_volume
                logger.info(f"开多，净多头[{position.pos}]不足目标手数[{self.volume}]，优先平空(昨)[{close_volume}]手，剩余开多[{open_volume}]手")
                if close_volume >0:
                    cmds.append(
                        OrderCmd(
                            symbol=self.symbol,
        ...
        if cmds and len(cmds) > 0:
            await self.send_order_cmds(cmds)
    ...
    def _on_cmd_change(self, cmd: OrderCmd):
        ...
        if cmd.filled_volume > 0:
            ...
            position.update_position(trade_data)
            ...
            logger.info(
                ...
                f"当前持仓 多={position.pos_long}({position.pos_long_td or 0}/{position.pos_long_yd or 0}) "
                f"空={position.pos_short}({position.pos_short_td or 0}/{position.pos_short_yd or 0})"
            )
    ...
    def calc_position_profit(self, symbol: str, last_price: float, multiple: int = 1) -> float:
        """
        计算指定合约的持仓盈亏（查询时使用，不保存到持仓对象）
        ...
        if not position or last_price <= 0 or multiple <= 0:
            return 0.0
    ...
    # ==================== 交易接口 ====================
    async def send_order_cmds(self, cmds: List[OrderCmd]):
        """发送报单指令（支持多指令）"""
