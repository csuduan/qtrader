...
class OrderCmdExecutor:
    ...
    def register(self, cmd: "OrderCmd") -> None:
        ...
        # 获取合约信息，设置最小开仓手数
        contract = self._trading_engine.get_contract(cmd.symbol)
        if contract and contract.min_open_volume:
            cmd.min_volume = contract.min_open_volume

        # 拆单
        try:
            cmd.split(pos)
        except Exception as e:
            self.logger.error(f"拆单失败: {e}")
            cmd.close(f"拆单失败:{e}")
