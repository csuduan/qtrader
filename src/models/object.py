...
class PositionData(BaseModel):
    ...
    symbol: str = Field(..., description="合约代码")
    exchange: Exchange = Field(..., description="交易所")
    multiple: int = Field(0, description="合约乘数")

    pos_long_yd: Optional[int] = Field(0, description="昨仓多头持仓数量")
    pos_short_yd: Optional[int] = Field(0, description="昨仓空头持仓数量")
    pos_long_td: Optional[int] = Field(0, description="今仓多头持仓数量")
    pos_short_td: Optional[int] = Field(0, description="今仓空头持仓数量")

    # 成本
    hold_price_long: Optional[float] = Field(0, description="多头持仓均价")
    hold_price_short: Optional[float] = Field(0, description="空头持仓均价")
    hold_cost_long: Optional[float] = Field(0, description="多头持仓成本(用于辅助计算均价)")
    hold_cost_short: Optional[float] = Field(0, description="空头持仓成本(用于辅助计算均价)")
    # 盈亏
    ...
    auto_updated: bool = Field(default=False, description="是否自动更新")
    ...
    def close_profit(self) -> float:
        """平仓盈亏"""
        return self.close_profit_long + self.close_profit_short
    
    @property
    def hold_profit(self) -> float:
        """持仓盈亏"""
        return self.hold_profit_long + self.hold_profit_short

    @classmethod
    def default(cls, symbol: str = "", exchange: Exchange = Exchange.NONE, multiple: int = 0):
        """创建空持仓实例"""
        return cls(
            symbol=symbol,
            exchange=exchange,
    ...
    def update_position(self, data: TradeData|TickData) -> None:
        """根据成交更新持仓"""
        ...
        if isinstance(data, TickData):     
            ...
            self.hold_profit_long = (self.last_price - self.hold_price_long) * self.pos_long * self.multiple
            self.hold_profit_short = (self.hold_price_short - self.last_price) * self.pos_short * self.multiple
            self.updated_at = data.datetime
        ...
        if isinstance(data, TradeData):
            ...
            if trade.offset == Offset.OPEN:
                ...
                if trade.direction == Direction.BUY:
                    # 开多 
                    cost = self.hold_price_long * self.pos_long + price * volume
                    self.pos_long_td += volume
                    self.hold_price_long = round(cost / self.pos_long, 2)if self.pos_long > 0 else 0.0
                else:
                    # 开空
                    cost = self.hold_price_short * self.pos_short + price * volume
                    self.pos_short_td += volume
                    self.hold_price_short = round(cost / self.pos_short, 2)if self.pos_short > 0 else 0.0
            else:
                # 平仓：减少持仓
                if trade.direction == Direction.BUY:
                    # 平空（买入平空）
                    if self.pos_short > 0:
                        ...
                        # 处理今仓/昨仓
                        pos_short_td = self.pos_short_td or 0
                        pos_short_yd = self.pos_short_yd or 0
                        if trade.offset == Offset.CLOSETODAY:
                            # 平今仓
                            self.pos_short_td = max(0, pos_short_td - volume)
                        else:
                            # 平昨仓（优先减昨仓）
                            yd_to_close = min(pos_short_yd, volume)
                            self.pos_short_yd = max(0, pos_short_yd - yd_to_close)
                            remaining = volume - yd_to_close
                            if remaining > 0:
                                self.pos_short_td = max(0, pos_short_td - remaining)

                        # 更新平仓盈亏
                        self.close_profit_short += (
                            (self.hold_price_short - price) * volume * self.multiple
                        )
                else:
                    # 平多（卖出平多）
                    if self.pos_long > 0:
                        # 处理今仓/昨仓
                        pos_long_td = self.pos_long_td or 0
                        pos_long_yd = self.pos_long_yd or 0
                        if trade.offset == Offset.CLOSETODAY:
                            # 平今仓
                            self.pos_long_td = max(0, pos_long_td - volume)
                        else:
                            # 平昨仓（优先减昨仓）
                            yd_to_close = min(pos_long_yd, volume)
                            self.pos_long_yd = max(0, pos_long_yd - yd_to_close)
                            remaining = volume - yd_to_close
                            if remaining > 0:
                                self.pos_long_td = max(0, pos_long_td - remaining)
                        
                        # 更新平仓盈亏
                        self.close_profit_long += (
                            (price - self.hold_price_long) * volume * self.multiple
                        )
...
class AccountData(BaseModel):
    ...
    # 必需字段
    account_id: str = Field(..., description="账户ID")
    balance: float = Field(0, description="账户余额(动态)")
    available: float = Field(0, description="可用资金")
    pre_balance: Optional[float] = Field(None, description="昨日权益")
    static_balance: Optional[float] = Field(None, description="静态权益(昨日权益+出入金)")
...
class ContractData(BaseModel):
    ...
    # 扩展字段
    extras: Dict[str, Any] = Field(default_factory=dict)
    min_open_volume: Optional[int] = Field(1, description="最小开仓手数")
