...
class RsiStrategy(BaseStrategy):
    ...
    def __init__(self, strategy_id: str, config: dict):
        ...
        self.trade_start_time = self._parse_time(self._get_str_param(config, ["StartTime", "trade_start_time"], "09:30:00"))
        self.trade_end_time = self._parse_time(self._get_str_param(config, ["EndTime", "trade_end_time"], "14:50:00"))
        self.force_exit_time = self._parse_time(self._get_str_param(config, ["ForceExitTime", "force_exit_time"], "14:55:00"))
    ...
    def _generate_signal(self) -> int:
        ...
        if rsi_long < (100 - self.rsi_long_threshold) and rsi_short < (100 - self.rsi_short_threshold):
    ...
    def _check_exit_conditions(self, current_price: float) -> Optional[str]:
        ...
        if ret >= self.take_profit_pct:
            logger.info(f"触发止盈: entry={self.entry_price}, current={current_price}, ret={ret:.4f}")
            return "TAKE_PROFIT"

        # 止损
        if ret <= -self.stop_loss_pct:
            logger.info(f"触发止损: entry={self.entry_price}, current={current_price}, ret={ret:.4f}")
            return "STOP_LOSS"
    ...
    def _close_position(self, reason: str):
        ...
        try:
            ...
            if order_id:
                logger.info(f"平仓成功: reason={reason}, side={self.position_side}, order_id={order_id}")
    ...
    def on_trade(self, trade):
        """成交回调"""
        logger.info(f"策略 [{self.strategy_id}] 成交: {trade.trade_id} {trade.symbol} "
