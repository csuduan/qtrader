      ...
      <el-form :model="signalForm" label-width="140px" v-if="strategy">
        ...
        >
          ...
          <el-table-column label="多头均价" width="130">
            <template #default="{ row }">
              {{ row.hold_price_long?.toFixed(2) || '-' }}
            </template>
          </el-table-column>
          <el-table-column label="空头均价" width="130">
            <template #default="{ row }">
              {{ row.hold_price_short?.toFixed(2) || '-' }}
            </template>
          </el-table-column>
    ...
    >
      <el-form :model="positionDialogForm" label-width="120px" ref="positionFormRef" :rules="positionFormRules">
        ...
        <el-form-item label="持仓均价">
          <el-input-number v-model="positionDialogForm.hold_price_long" :min="0" :step="0.01" :precision="2" style="width: 100%" />
        </el-form-item>
        <el-form-item label="平仓盈亏">
          <el-input-number v-model="positionDialogForm.close_profit_long" :step="0.01" :precision="2" style="width: 100%" />
        </el-form-item>
        ...
        <el-form-item label="持仓均价">
          <el-input-number v-model="positionDialogForm.hold_price_short" :min="0" :step="0.01" :precision="2" style="width: 100%" />
        </el-form-item>
                <el-form-item label="平仓盈亏">
          <el-input-number v-model="positionDialogForm.close_profit_short" :step="0.01" :precision="2" style="width: 100%" />
        </el-form-item>
...
function initEditablePositions() {
  ...
  if (positions.length > 0) {
    ...
    editablePositions.value = [{
      ...
      hold_price_long: strategy.value.pos_price || 0,
      hold_price_short: strategy.value.pos_price || 0,
      position_profit: 0,
      close_profit: 0,
      _editing: false
    }]
...
function handleEditPosition(row: any) {
  ...
  positionDialogForm.value = {
    ...
    hold_price_long: row.hold_price_long || 0,
    hold_price_short: row.hold_price_short || 0,
    close_profit_long: row.close_profit_long || 0,
    close_profit_short: row.close_profit_short || 0
  }
  positionDialogVisible.value = true
}
...
async function handleSavePositionDialog() {
  ...
  try {
    ...
    const positionData = {
      ...
      hold_price_long: positionDialogForm.value.hold_price_long || 0,
      hold_price_short: positionDialogForm.value.hold_price_short || 0,
      close_profit_long: positionDialogForm.value.close_profit_long || 0,
      close_profit_short: positionDialogForm.value.close_profit_short || 0
    }
